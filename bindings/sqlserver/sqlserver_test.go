...
// 4. `export SQLSERVER_TEST_CONN_URL="sqlserver://daprtest@localhost:5432/daprtest"``
// 5. `go test -v -count=1 ./bindings/sqlserver -run ^TestSQLServerIntegration`

func TestSQLServerIntegration(t *testing.T) {
	url := os.Getenv("SQLSERVER_TEST_CONN_URL")
	if url == "" {
		t.SkipNow()
	}
